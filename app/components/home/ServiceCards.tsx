  ...
  return (
    <section id="services-overview" className="bg-cream py-24 sm:py-28 lg:py-36">
      <Container size="wide">
        ...
        {/* Cards */}
        <RevealGroup className="mt-16 grid gap-px bg-slate-line border border-slate-line sm:grid-cols-1 lg:grid-cols-3" stagger={0.1}>
          {services.map((s) => (
            <RevealItem key={s.no}>
              <div className="group flex h-full flex-col bg-ivory p-8 transition-colors duration-300 hover:bg-white lg:p-10">
                {/* Number + title */}
                <div>
                  <p className="font-display text-5xl tabular-nums text-slate-soft/40 transition-colors duration-300 group-hover:text-slate-soft/60">
                    {s.no}
                  </p>
                ...
                <div className="mt-8 border-t border-slate-line pt-6">
                  <Link
                    ...
                    className="inline-flex items-center gap-2 text-sm font-medium text-navy transition-colors hover:text-slate"
                  >
                    Full breakdown
